...
float ax_g, ay_g, az_g,                                 // Valores do acelerômetro em G
      ...
      gx_deg_filt, gy_deg_filt, gz_deg_filt,            //Valores do giroscópio em âgulos filtrados 
      gx_deg_old, gy_deg_old, gz_deg_old,               // Valores da medida anterior de g_deg
      gx_deg_filt_old,gy_deg_filt_old,gz_deg_filt_old,   //Valores da medida anterior de g_deg_filt
...
void loop(){
  ...
  ax_g = (double)ax/SCALE_ACCEL;              // Conversão dos dados de aceleração para 2g
  ay_g = (double)ay/SCALE_ACCEL;              //
  az_g = (double)az/SCALE_ACCEL;              // ----------------------------------------
...
// Início do cálculo dos ângulos pelo giroscópio-----------
  ...
  gx_deg_filt = gx_deg - gx_deg_old + ALPHA*gx_deg_filt_old;                                                        //y(n) = x(n) - x(n-1) +a*y(n-1);
  gx_deg_filt_old = gx_deg_filt;
  gx_deg_old = gx_deg;                            
  ...
  gy_deg_filt = gy_deg - gy_deg_old + ALPHA*gy_deg_filt_old;                                                        //y(n) = x(n) - x(n-1) +a*y(n-1);
  gy_deg_filt_old = gy_deg_filt;
  gy_deg_old = gy_deg;                             
    
  gz_deg += 0.5*(gz_dps + gz_old)*dt;                                 // Cálculo integral de posição angular no eixo z;
  gz_old = gz_dps;  
 // gz_deg = (1*gz_deg - 0.006*gz_deg_old) + 0.013;
  gz_deg_filt = gz_deg - gz_deg_old + ALPHA*gz_deg_filt_old;                                                        //y(n) = x(n) - x(n-1) +a*y(n-1);
  gz_deg_filt_old = gz_deg_filt;
  gz_deg_old = gz_deg;     
  gz_deg_old = gz_deg;
...
// Início do cálculo dos ângulos pelo acelerômetro --------------------------------------------------------------------------
  ax_deg = (float)atan2(ay_g,az_g)*180/3.14;///atan(ay_g/sqrt(pow(ax_g,2) + pow(az_g,2)))*(180.0/3.14);         // Cálculo dos ângulos do acelerômetro por arcotangente
  ay_deg = (float)atan2(ax_g,az_g)*180/3.14;//atan(-ax_g/sqrt(pow(ay_g,2) + pow(az_g,2)))*(180.0/3.14);        //  "   "
  //ax_deg = ax_g;
  //ay_deg = ay_g;
  
  //ax_deg = (ALPHA_X*ax_deg + (1.0 - ALPHA_X)*ax_deg_old);             // Filtro Passa-Baixas feito na intuição
  ax_deg = (1-ALPHA)*ax_deg + ALPHA*ax_deg_old;                                       // y(n) = x(n-1) - a*x(n-1) +a*y(n-1); 
  //ay_deg = (ALPHA_Y*ay_deg + (1.0 - ALPHA_Y)*ay_deg_old) + Y_OFFSET_ACCEL;  // Filtro Passa-Baixas feito na intuição
  ay_deg = (1-ALPHA)*ay_deg + ALPHA*ay_deg_old;
  ax_deg_old = ax_deg;
  ay_deg_old = ay_deg;
  
  ang_x = ax_deg + gx_deg_filt;                                            // Filtro complementar
  ang_y = ay_deg + gy_deg_filt;                                            // Filtro complementar 
  ang_z = gz_deg_filt;                                                     // Angulo no eixo z não possui complemento
...
//-----------------------Inicio da gravação----------------------//
// Aqui são passados os dados para o vetor, que será gravado no SD
  enfileirar((uint16_t)(ax_g*1000));  
  enfileirar((uint16_t)(ay_g*1000));
  enfileirar((uint16_t)(az_g*1000));
  enfileirar((uint16_t)(ang_x*100));
  enfileirar((uint16_t)(ang_y*100));
  enfileirar((uint16_t)(ang_z*100));  
